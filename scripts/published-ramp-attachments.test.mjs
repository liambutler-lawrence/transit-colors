...
test('published ramp repairs retain their curves and both mainline connections across the network', async () => {
  const audit = JSON.parse(
    await readFile(
      new URL('./fixtures/ramp-attachment-audit.json', import.meta.url),
      'utf8',
    ),
  );
  assert.ok(
    audit.connectors.some(
      (part) =>
