...
async function main() {
  ...
  for (const stopTime of stopTimes) {
    ...
    const serviceDays =
      stopSchedule.services.get(serviceKey) ?? DAY_COLUMNS.map(() => new Map());
  ...
  for (const station of openStations) {
    ...
    const nearbyStops = transitStops
      ...
      .filter(
        ({ stop, meters }) =>
          meters <= 140 || (meters <= 650 && namesMatch(properties.name, stop.name)),
      )
    ...
    const matches = nearbyStops.filter(({ meters }) => meters <= maximumMatchDistance);
    ...
    for (const { stop, meters } of matches) {
      ...
      for (const [serviceKey, serviceDays] of stop.schedule.services) {
        const mergedDays = services.get(serviceKey) ?? DAY_COLUMNS.map(() => new Map());
        for (let day = 0; day < DAY_COLUMNS.length; day += 1) {
